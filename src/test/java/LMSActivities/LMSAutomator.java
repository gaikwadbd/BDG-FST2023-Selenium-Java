...
public class LMSAutomator extends BaseTest{
	...
	public void Login() {
		...
		Actions builder = new Actions(getDriver());
        builder.click(userTextBox).sendKeys(lms_selenium_project.Configuration.USER_NAME).
        click(passTextBox).sendKeys(lms_selenium_project.Configuration.PASSWORD).click(submitButton);
        Action compAction = builder.build();
		compAction.perform();
